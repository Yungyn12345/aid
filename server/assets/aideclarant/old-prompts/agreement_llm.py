...
class AgreementLLM(TNVED_API_ABC):
    """
    Анализ PDF договора через Perplexity с json_schema (AGREEMENT_SCHEMA).
    """

    async def request(self, pdf_bytes: bytes) -> str:
        """
        pdf_bytes — содержимое PDF-фаила договора.
        """

        # Перегоняем PDF → base64 без data: prefix (как в Telegram-боте)
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": AGREEMENT_INSTRUCTION_RU},
                        {
                            "type": "file_url",
                            "file_url": {"url": b64},
            ...
            "response_format": {
                "type": "json_schema",
                "json_schema": {"schema": AGREEMENT_SCHEMA}
            },
            "temperature": 0.0,
            "web_search_options": {"search": False},
        }

        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.post(API_URL, data=json.dumps(payload)) as resp:
