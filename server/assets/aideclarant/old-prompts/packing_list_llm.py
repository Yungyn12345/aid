# external_api/packing_list_llm.py
...
class PackingListLLM(TNVED_API_ABC):
    """
    Анализ PDF упаковочного листа через Perplexity с json_schema (PACKING_LIST_SCHEMA).
    """

    async def request(self, pdf_bytes: bytes) -> str:
        """
        pdf_bytes — содержимое PDF-файла упаковочного листа.
        """
        ...
        payload = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": PL_INSTRUCTION_RU},
                        {
                            "type": "file_url",
                            "file_url": {"url": b64},
            ...
            "response_format": {
                "type": "json_schema",
                "json_schema": {"schema": PACKING_LIST_SCHEMA}
            },
            "temperature": 0.0,
