...
class CmrLLM(TNVED_API_ABC):
    """
    Анализ PDF CMR через Perplexity с json_schema (CMR_SCHEMA).
    """
    ...
    async def request(self, pdf_bytes: bytes) -> str:
            ...
            "messages": [
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": CMR_INSTRUCTION_RU},
                        {
                            "type": "file_url",
                            "file_url": {"url": b64},
                            "file_name": "cmr.pdf"
        ...
        # как и в других LLM-классах — возвращаем content (строку JSON)
