...
class InvoiceLLM(TNVED_API_ABC):
    """
    Анализ PDF инвойса через Perplexity с json_schema (INVOICE_SCHEMA).
    """

    async def request(self, pdf_bytes: bytes) -> str:
        """
        pdf_bytes — содержимое PDF-файла инвойса.
        """
        ...
        payload: dict[str, Any] = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": INVOICE_INSTRUCTION_RU},
                        {
                            "type": "file_url",
                            "file_url": {"url": b64},
            ...
            "response_format": {
                "type": "json_schema",
                "json_schema": {"schema": INVOICE_SCHEMA},
            },
            "temperature": 0.0,
        ...
        # Perplexity возвращает JSON как строку в поле content
