...
def _build_ssl_context() -> ssl.SSLContext | bool:
    ...
    if GIGACHAT_CA_BUNDLE_FILE:
        return ssl.create_default_context(cafile=GIGACHAT_CA_BUNDLE_FILE)
...
async def _run_with_model_fallback(models: list[str], operation_name: str, func):
    errors: list[str] = []
        ...
        async with aiohttp.ClientSession(headers=headers) as session:
            async with session.post(
                ...
                try:
                    ...
                    raise ProviderAPIError(f"GigaChat OAuth returned non-JSON response: {text[:500]}") from exc

                if resp.status >= 400:
                    message = ((data.get("error") or {}).get("message")) or text
                    raise ProviderAPIError(f"GigaChat OAuth error {resp.status}: {message}")
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        async with session.post(
            ...
            try:
                ...
                raise ProviderAPIError(f"GigaChat returned non-JSON response: {text[:500]}") from exc

            if resp.status >= 400:
                message = ((data.get("error") or {}).get("message")) or text
                raise ProviderAPIError(f"GigaChat API error {resp.status}: {message}")

            return data


async def _repair_json_response(*, raw_text: str, schema: dict, context: str, model: str) -> dict:
    payload = {
        "model": model,
        "messages": [
            {
    ...
    return _parse_json_content(_extract_json_text(data), context=f"{context}_repair")


async def request_document_json(*, pdf_bytes: bytes, instruction: str, schema_name: str, file_name: str, schema: dict) -> str:
    ...
    async def _attempt(model: str) -> str:
        ...
        data = await _request(payload)
        parsed = _parse_json_content(_extract_json_text(data), context=file_name)
        ...
        parsed = _validate_payload(parsed, schema=schema, context=file_name)
        return json.dumps(parsed, ensure_ascii=False)

    return await _run_with_model_fallback(DOCS_MODEL_CHAIN, f"document extraction for {file_name}", _attempt)


async def request_tnved_json(*, prompt: str, instruction: str, schema_name: str, schema: dict) -> str:
    del schema_name
    ...
    async def _attempt(model: str) -> str:
        ...
        data = await _request(payload)
        raw_text = _extract_json_text(data)
        try:
            parsed = _parse_json_content(raw_text, context="tnved_classification")
            parsed = _normalize_tnved_payload(parsed)
            parsed = _validate_payload(parsed, schema=schema, context="tnved_classification")
            ...
            parsed = _normalize_tnved_payload(repaired)
            parsed = _validate_payload(parsed, schema=schema, context="tnved_classification")
        return json.dumps(parsed, ensure_ascii=False)

    return await _run_with_model_fallback(TNVED_MODEL_CHAIN, "tnved classification", _attempt)
